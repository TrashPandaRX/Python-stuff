...
# map2.py is a followup to map.py where this is currently a continuation of my notes
...
fgv = folium.FeatureGroup(name = "Volcanic Activity")
...
# PS you didnt need to use feature group for population density, volcanoes however you needed feature group lest you would have ended up with 62 individuals which is messy.
fgpop = folium.FeatureGroup(name = "Population Density")

# this is so fucking weird, i had to put braces around each fillColor: <Color> combination AND add an ADDITIONAL else statement to get the fucking code to stop screaming at me... wtf (oh and i had to tack .read() to the end of the open() function)
fgpop.add_child(folium.GeoJson(data = open('Webmap_datasources/world.json', 'r', encoding = 'utf-8-sig').read(), style_function = lambda x: {'fillColor' : 'blue'} if x['properties']['POP2005'] < 10000000 else ({'fillColor' : 'orange'} if 10000000 <= x['properties']['POP2005'] < 20000000 else {'fillColor' : 'red'})))


map_volcanoes.add_child(fgv)
map_volcanoes.add_child(fgpop)

map_volcanoes.add_child(folium.LayerControl())    # newest addition (6/18/22)
# so because you added the volcano locations and the population at the same time via .add_child(fg) you need to break them up into individual occurances otherwise
# layer control will assume both points and shaded zones are one layer (turn fg into fgv and fgpop for volcanic map and population shading respectively)
map_volcanoes.save("Map_layercontrol_json.html")
