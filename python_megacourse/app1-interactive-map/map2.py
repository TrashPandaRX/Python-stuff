...
#style_function = lambda x: 'fillColor':'green' if x['properties']['POP2005'] < 10000000 else 'fillColor':'orange' if 10000000 <= x['properties']['POP2005'] < 20000000))
...
# PS YOU MUST SAY 'fillColor' : 'color' for EVERY CONDITIONAL idk how the fuck the guy who wrote this tutorial didnt get reamed by these errors unless they changed it after the fact.
fg.add_child(folium.GeoJson(data = open('Webmap_datasources/world.json', 'r', encoding = 'utf-8-sig').read(), style_function = lambda x: {'fillColor' : 'blue'} if x['properties']['POP2005'] < 10000000 else ({'orange'} if 10000000 <= x['properties']['POP2005'] < 20000000 else {'red'})))


map_volcanoes.add_child(fg)
map_volcanoes.add_child(folium.LayerControl())    # newest addition (6/18/22)
# so because you added the volcano locations and the population at the same time via .add_child(fg) you need to break them up into individual occurances otherwise layer control will assume both points and shaded zones are one layer
map_volcanoes.save("Map_popmap_layercontrol_json.html")
